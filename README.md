...
截至当前记录，**没有下载第十七轮候选到板卡**。两板仍运行第十六轮各自 ELF 及用户原 bitstream。
...
已请求将下载器USB和JTAG两端断开至少10秒，先只把USB接到另一个主板直连接口，板卡继续上电；
需先恢复下载器枚举，不能继续盲目下载候选或把这一USB故障归因于RF逻辑。
此前双机原文件完整复核分别424/425项，哈希差异0；没有候选下载、没有改变原ELF/bit/源工程。
接收端GUI回传已恢复为192.168.1.100:15002，并收到RXCFG确认；JTAG端口读回仍待连接恢复。
