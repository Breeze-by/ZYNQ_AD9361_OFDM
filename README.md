...
每次 PS 准备通过 MM2S 把一个聚合块送入 PL 前，会先 arm 一个 `8192` 字节 S2MM 捕获窗口。S2MM 完成后，PS 会 invalidate RX buffer，并按当前 `tx_transfer` 长度比较 RX 前段和 TX buffer。当前阶段只打印和比较，不把回环数据 UDP 发回 PC。
...
S2MM done id=1 capture=8192 tx_transfer=2880 cmp_len=2880 irq=0x... sr=0x... rx_crc=0x... tx_crc=0x... cmp=OK done=1
S2MM done id=1 capture=8192 tx_transfer=2880 cmp_len=2880 irq=0x... sr=0x... rx_crc=0x... tx_crc=0x... cmp=DIFF first_diff=...
S2MM rx_head ...
S2MM tx_head ...
