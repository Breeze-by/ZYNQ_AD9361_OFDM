...
## 2026-09-12 第十七轮：包头保护、业务 DATA 降功率对照（进行中）
...
截至当前记录，**没有下载第十七轮候选到板卡**。正式保留文件仍为第十六轮各自 ELF 及用户原 bitstream；
USB/JTAG恢复期间发现接收板未加载bitstream，不能再据磁盘版本断言两板都正在运行原程序，恢复状态见下文。
...
- TX22/RX66、2.2 GHz、40 MSPS、原搬运时钟、63、RF 门限、400 KiB/s、chunk1024/window1、PC->PS CRC/ACK 与无 RF 重传均保持。
