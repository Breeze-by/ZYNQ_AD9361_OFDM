...
下面这张图只保留当前系统的大致流程。汇报时可以重点说明：PC 负责上层文件/视频封装与最终恢复展示；PS 负责 UDP 协议、ACK、聚合和 DMA 调度；PL 负责 OFDM 发送/回环恢复；AIR0/AIRV 都是 PC-only 格式，PS/PL 不解析。

```mermaid
flowchart LR
    subgraph PC1["PC 发送端"]
        A["选择输入<br/>Test Data / File / MP4"]
        B{"发送模式"}
        C["AIR0 文件/测试<br/>可精确恢复"]
        D["AIRV 实时视频<br/>H.264 分片 + 预览"]
        E["raw 原始字节"]
        F["UDP DATA<br/>net_data_header_t + wire payload"]
    end

    subgraph ZYNQ["Zynq PS"]
        G["lwIP UDP 接收<br/>session / seq / CRC 校验"]
        H["ACK / 重传配合<br/>OK / BUSY / PENDING / CRC error"]
        I["DDR 聚合<br/>3000B payload block"]
        J["AXI DMA MM2S<br/>送入 PL"]
        K["AXI DMA S2MM<br/>接收 PL 回环结果"]
        L["UDP loopback return<br/>分片回传 PC 接收端"]
    end

    subgraph PL["PL + AD9361"]
        M["tx_intf / openofdm_tx"]
        N["AD9361 / OFDM loopback"]
        O["openofdm_rx / rx_intf"]
    end

    subgraph PC2["PC 接收端"]
        P["receiver_gui 注册回传目标<br/>RXCFG"]
        Q["接收 loopback UDP<br/>连续性和 CRC 检查"]
        R{"识别 payload"}
        S["AIR0<br/>恢复原文件并校验 file_crc"]
        T["AIRV<br/>实时组帧 + PyAV 解码"]
        U["AIRV Preview<br/>独立 1280x720 窗口"]
        V["日志与统计<br/>DONE / VIDEO / Preview 指标"]
    end
    ...
    class A,B,C,D,E,F,P,Q,R,S,T,U,V pc;
    class G,H,I,J,K,L ps;
    class M,N,O pl;
    class C,D,E,S,T mode;
```
