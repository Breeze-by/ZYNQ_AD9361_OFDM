...
# Explicitly stop transmission/release JTAG and serial tools before use.
# RF defaults are unchanged; historical runtime DC44 is NOT baked into this ELF.
...
foreach name {net_snr_request net_snr_poll} {symbol $name}
proc u32 {a} {return [expr {[mrd -value $a] & 0xffffffff}]}
proc step {s} {puts "SNR_DEFAULTS_LOAD_STEP $s"; flush stdout}
step connect
...
foreach {a expected} {
    ...
    puts [format "SNR_DEFAULTS_RUNTIME_REG %08X=%08X" $a $actual]
}
if {[u32 [symbol gain]] != $expected_gain || [u32 [symbol txatt]] != $expected_att} {error "Runtime role changed"}
puts "SNR_DEFAULTS_LOADED role=$role gain=[u32 [symbol gain]] txatt=[u32 [symbol txatt]] guard=32 shift=4 no_ram_patch=1"
disconnect
