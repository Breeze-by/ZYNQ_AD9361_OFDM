...
static int UART_SendAllInternal(const uint8_t *data, uint32_t len)
{
    uint32_t total_sent = 0;

    while (total_sent < len)
    {
        int sent_len = XUartPs_Send(&UartPs1, (uint8_t *)&data[total_sent], len - total_sent);
...
void UART_SendString(const char *str)
{
    ...
    UART_SendAllInternal((const uint8_t *)str, (uint32_t)strlen(str));
}
...
int UART_Printf(const char *fmt, ...)
{
    ...
    return UART_SendAllInternal((const uint8_t *)buffer, (uint32_t)len);
}
...
int UART_SendBytes(const uint8_t *data, uint32_t len)
{
    ...
    sent_len = UART_SendAllInternal(data, len);
