...
- 调试 AD9361 RF 回环要分阶段做。当前链路不是纯 PL 数字回环：PL 侧 OFDM 调制后送 AD9361 TX，经 SMA 线直连 AD9361 RX，再进入 PL OFDM RX/解调，后续仍通过 S2MM 回 PS 并 UDP 回传 PC。由于本地无法板级验证，不要一次性写完大功能；先加可观察日志，让用户上板跑并回传串口输出，再根据日志继续改。
- 默认已关闭逐帧 S2MM 大段日志，降低 UART 对实时链路的影响。需要用户反馈时，优先要 `RXCFG loopback peer`、`UDP RX reset`、`STAT rate/state`、`DMA stall timeout/recovery`、`S2MM error`、`MM2S error`；只有临时把 `NET_LOOPBACK_S2MM_LOG_FIRST_BLOCKS` 或 `NET_LOOPBACK_S2MM_LOG_DIFF_ALWAYS` 打开时，才要求复制 `S2MM start/done`、`S2MM rx_head`、`S2MM tx_head`、`S2MM rx_hdr`、`S2MM payload_magic`、`S2MM air0`、`LB UDP sent`。如果涉及 PC 端回传验证，还要让用户复制接收 GUI 日志里的 `RX target registered ...`、AIR0 的 `PROGRESS rx=... crc=... len=... gaps=... air=... air_rx=... pending_air=... bad_hdr=... bad_payload=... bad_meta=... dup=... got_last=...`、`INCOMPLETE ... missing_seq=... bad_payload_seq=... bad_meta_seq=...`、`DONE ... gaps=... air=... air_rx=... miss=... file_crc=... file_id=... file_size=... total_packets=... got_last=... saved=... missing_seq=... bad_payload_seq=... bad_meta_seq=...`，以及 AIRV 的 `VIDEO frame_rx=... frame_show=... frame_drop=... frag_rx=... frag_missing=... bad_hdr=... bad_meta=... bad_frag_crc=... bad_frame_crc=... keyframe_rx=... waiting_keyframe=... fps=... latency_ms=...`、`VIDEO_FRAME ...`、`VIDEO_DONE ...`、`DONE VIDEO ...` 行。
...
- 当前已开启 AD9361 RF 回环后的 PL->PS S2MM 调试和 UDP 回传：每次 MM2S 前 arm `8192` 字节 S2MM 捕获窗口，完成后跳过 RX 前 16 字节前缀，按聚合块真实 `payload_len` 比较 RX payload 和 TX buffer；`tx_transfer` 只是 8 字节对齐后的 DMA 长度，尾部 padding 不参与 payload 比较。随后 PS 用 magic `0x304B424C` 的 loopback UDP 包把 payload 分片发回已注册的接收 GUI/CLI。当前加了 `NET_DMA_STALL_TIMEOUT_US = 6000` 的发送/RF 回环 watchdog：真实空口 RX 或 S2MM 不完成时会打印 `DMA stall timeout ...` 和 `DMA stall recovery ...`，重置 AXI DMA、释放当前聚合块并继续下一块，不再让接收端无信号把 PC 发送端拖到 `BUSY` 重试耗尽。接收端按 `stream_offset + chunk_offset` 恢复连续 payload，检查分片 CRC、长度和缺口，并把图片/视频等原始文件保存到 `output`；如果存在缺口，接收端应报 `INCOMPLETE` 且不保存带洞文件。AIR0 最终缺包时接收端会在 `INCOMPLETE` / `DONE` 中输出 `missing_seq=...`，表示缺失 `packet_seq` 范围；payload CRC 错误和 AIR0 元数据不一致会输出 `bad_payload_seq=...`、`bad_meta_seq=...`。`PROGRESS pending_air=...` 只是当前尚未收到的包数，不是最终丢包数。
- 当前临时启用 `NET_LOOPBACK_RETURN_SOURCE_TX_BUFFER` 诊断模式：PS 不启动 MM2S/S2MM，而是把聚合后的 TX buffer 直接用 loopback UDP 发回接收 GUI，启动日志会显示 `Loopback return source=TX_BUFFER diagnostic, MM2S/S2MM bypassed`，每块会打印 `TXECHO return ... first=0x30524941 ...`。该模式只用于证明 PC 发送、PS 接收/聚合、PS UDP 回传和 PC 接收恢复是否正常；验证完需要切回 `NET_LOOPBACK_RETURN_SOURCE_S2MM` 才能测试 AD9361 RF 链路。
- 由于当前 TX/RX 已解耦，S2MM 收到的帧不一定对应当前刚送入 MM2S 的聚合块。PS 侧现在会在 S2MM buffer 前部扫描 AIR0/AIRV magic；若识别到 AIR0，会打印 `S2MM air0 ... desync=...`，并用 AIR0 `packet_seq * chunk_bytes` 推导 UDP 回传 `stream_offset`，避免把延迟堆积的 RX 帧错误标成当前 TX block offset。若 `S2MM payload_magic none`，说明返回数据前部没有可识别 AIR0/AIRV 头，应继续查 PL/RF/RX 或返回格式偏移。
- 为验证 UART 打印是否影响接收，当前默认 `NET_LOOPBACK_S2MM_LOG_FIRST_BLOCKS=0`、`NET_LOOPBACK_S2MM_LOG_INTERVAL_BLOCKS=0`、`NET_LOOPBACK_S2MM_LOG_DIFF_ALWAYS=0`，即使 `cmp=DIFF` 也不强制打印整块诊断；临时需要查内容时再打开。
