...
- 当前已开启 PL->PS S2MM 调试和 UDP 回传，默认 `NET_LOOPBACK_RETURN_SOURCE=NET_LOOPBACK_RETURN_SOURCE_S2MM`：每次 MM2S 前 arm `8192` 字节 S2MM 捕获窗口，完成后扫描前 `2048` 字节 AIR0/AIRV magic，跳过 RX 前缀，按聚合块真实 `payload_len` 比较 RX payload 和 TX buffer；`tx_transfer` 只是 8 字节对齐后的 DMA 长度，尾部 padding 不参与 payload 比较。随后 PS 用 magic `0x304B424C` 的 loopback UDP 包把 payload 分片发回已注册的接收 GUI/CLI。为排查 PL 数字回环开头约 8 个 S2MM block 超时，`NET_DMA_STALL_TIMEOUT_US` 已从 `6000` 临时放宽到 `20000`：若块在 6～20 ms 完成则说明旧 watchdog 过短；若仍需多次 `DMA stall recovery` 才恢复，则优先定位 PL RX/S2MM 启动状态。
- 成功完成的 `S2MM diag` / `S2MM done` 包含 `wait_us`，表示 arm S2MM 到主循环观察到 `RxDone` 的时间；用前 16 块的分布确定最终 watchdog，不要只凭是否低于 20 ms。
